package com.epam.jwd.controller;

public class PageConstant {

    public static final String HOME_PAGE = "home";

    public static final String LOGIN_PAGE = "login";

    public static final String ERROR_PAGE = "error";

    public static final String REGISTER_PAGE = "register";

    public static final String OTP_PAGE = "user-otp";

    public static final String PROFILE_PAGE = "profile";

    public static final String PRODUCTS_PAGE = "products";

    public static final String ADD_PRODUCT_PAGE = "admin/add-product";

    public static final String EDIT_PRODUCT_PAGE = "admin/edit-product";

    public static final String SEARCH_PRODUCT_PAGE = "admin/search-product";

    public static final String VIEW_PRODUCTS_PAGE = "admin/view-products";

}
